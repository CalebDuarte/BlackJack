...
function deal() {
  if (bet <= 0) {
    setMessage('YOU CANT CHEAT THE HOUSE! Place a bet first!', 'lose');
    return;
  }
...
function doubleDown() {
  ...
  if (balance < bet) {
    setMessage('THE HOUSE GETS ITS MONEY NOT ENOUGH FUNDS!!', 'lose');
    return;
  }
...
//decide the winner and player/dealer payout
function resolveGame() {
  ...
  if (balance <= 0) {
    setTimeout(() => {
      ...
      updateBalance();
      setMessage("Looks like the HOUSE won get back in there...", 'push');
    }, 2000);
  }
  setButtons();
}
